...
# 중첩 자료구조 #(리스트 - 벡터, 배열, 데이터 프레임, 리스트. 자바 객체)
...
# vector는 같은 data type으로 구성돼요
# vector는 첨자형태로 access가 가능( [] )
# 첨자(index)의 시작은 1 #(다른 것은 0. R 차별)
...
var2 = rep(1:3, each=3)  # each는 생략이 불가능
...
var1[6]        # NA
var1[-1]       # "-"는 제외의 의미
var1[-c(1:3)]  # 50 100 #(R 차별. Python에서는 다름)
...
var1 + var4   # 1 2 3 1 2 
              # 5 6 7 8 9 
              # 6 8 10 9 11 # 동작은가능. warning

# vector간의 집합 연산 #(합집합,교집합 등)
# union() : 합집합
# intersect() : 교집합
# setdiff() : 차집합
...
# identical : 비교하는 두 vector의 요소의 개수. 순서, 내용이 같아야지 TRUE를 return
